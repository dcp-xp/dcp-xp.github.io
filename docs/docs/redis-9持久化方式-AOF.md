...
![AOF传播过程流程图](../images/AOF传播过程流程图.png)
...
协议文本生成之后， 它会被追加到 server.h/redisServer 结构的 aof_buf 末尾。
...
- 子线程没有在执行 SAVE ，并且：
  ...
  - 上次成功执行 SAVE 距今已经超过 1 秒，那么程序执行 WRITE 和 SAVE 。
    可以用流程图表示这四种情况：
    ![四种情况的判断流程](../images/四种情况的判断流程.png)
